const helper = {
  ...
  lineEquals: ([v1, w1], [v2, w2]) =>
    this.vertexEquals(v1, v2) && this.vertexEquals(w1, w2),

  getHoshis: function (width, height) {
  ...
  readjustShifts: function (shiftMap, vertex = null) {
    if (vertex == null) {
      for (let y = 0; y < shiftMap.length; y++) {
        for (let x = 0; x < shiftMap[0].length; x++) {
          this.readjustShifts(shiftMap, [x, y]);
        }
      }
    } else {
      let [x, y] = vertex;
  ...
  diffSignMap: function (before, after) {
    if (
      before === after ||
      before.length === 0 ||
